...
TEST_CASE("sm83 test suite without cb opcodes", "[sm83, cpu]") {
...
TEST_CASE("sm83 cb opcodes only", "[sm83, cpu]") {
  for (int i = 0; i <= 0xff; i++) {
    char file_name[11];
    sprintf(file_name, "cb %02x.json", i);
    std::string file_id{file_name};
    file_id = "./sm83/v1/" + file_id;
    std::ifstream f(file_id);

    DYNAMIC_SECTION("CB Test " << std::hex << i << std::dec) {
      json data = json::parse(f);
      for (auto test : data) {
        // load registers
        reg.pc = test["initial"]["pc"];
        ...
        // execute opcode
        REQUIRE(mmu.read_byte(reg.pc++) == 0xcb);
        cb.test();
