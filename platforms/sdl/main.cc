...
static void set_large_pixel(uint32_t *pixels, uint x, uint y,
                            uint32_t pixel_argb) {
...
static void process_events() {
  ...
  while (SDL_PollEvent(&event)) {
    ...
    case SDL_KEYDOWN:
      ...
      if (auto button_pressed = get_gb_button(event.key.keysym.sym);
          button_pressed) {
    ...
    case SDL_KEYUP:
      ...
      if (auto button_released = get_gb_button(event.key.keysym.sym);
          button_released) {
...
int main(int argc, char *argv[]) {
  ...
  log_info("Read %d KB from %s", rom_data.size() / 1024,
           cliOptions.filename.c_str());
